...
.hero-game-guide{position:absolute;left:50%;bottom:14px;transform:translateX(-50%);z-index:12;width:max-content;max-width:calc(100% - 36px);padding:10px 13px 9px;border:2px solid var(--ink);border-radius:14px;background:rgba(244,241,233,.94);box-shadow:4px 4px 0 var(--yellow);text-align:center;pointer-events:none;font-family:-apple-system,BlinkMacSystemFont,"Hiragino Sans","Yu Gothic",sans-serif}
.hero-game-guide b{display:block;font:950 9px/1 Arial,sans-serif;letter-spacing:.16em;margin-bottom:6px}
.hero-game-guide .start{display:block;font-size:13px;font-weight:900;line-height:1.2}
.hero-game-guide .controls{display:block;margin-top:5px;font-size:10px;font-weight:850;line-height:1.35;white-space:nowrap}
body.game-running .hero-game-guide .start{display:none}
body.game-running .hero-game-guide .controls{margin-top:0}
`;
document.head.appendChild(style);

const heroGuide=document.createElement('div');
heroGuide.className='hero-game-guide';
heroGuide.setAttribute('aria-hidden','true');
heroGuide.innerHTML='<b>HOW TO PLAY</b><span class="start">目玉をタップでスタート</span><span class="controls">←→ 指で左右になぞる　｜　↓ ページをスクロール</span>';
eye.appendChild(heroGuide);
