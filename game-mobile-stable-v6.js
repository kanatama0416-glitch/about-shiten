(function(){
  ...
  fetch('./game-core.js',{cache:'no-store'}).then(function(r){if(!r.ok)throw new Error('game core '+r.status);return r.text();}).then(function(code){
    ...
    code=code.replace(
      ...
      "for(const o of obstacles){if(o.b<ballY-ballR-70||o.t>ballY+ballR+70)continue;collide(o)}if(window.__ABOUT_MOBILE_TOUCH&&!steer)ballVX=0;releaseIfStuck(now);const g=goalHit();"
    );
    Function(code)();
