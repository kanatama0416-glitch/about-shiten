...
function updateIdleGuide(now){
 ...
 const sx=ballX-scrollX,sy=screenY,gap=12,margin=10;
 let x=sx+ballR+gap;
...
function goalHit(){const r=goal.getBoundingClientRect();const gx=r.left+r.width/2+scrollX,gy=r.top+r.height/2+scrollY;const rx=r.width*.29,ry=r.height*.25;const nx=(ballX-gx)/Math.max(1,rx-ballR*.35),ny=(ballY-gy)/Math.max(1,ry-ballR*.35);return nx*nx+ny*ny<=1?{gx,gy}:null}
function startDropGame(){if(gameRunning)return;gameRunning=true;secretHint.classList.remove('show');gameStartTime=performance.now();result.classList.remove('show');checkpointIndex=0;steer=false;key=0;manualScroll=false;const pr=pupil.getBoundingClientRect(),cx=pr.left+pr.width/2,cy=pr.top+pr.height/2,size=Math.max(36,Math.min(42,innerWidth*.105));ballR=size/2;ball.style.width=size+'px';ball.style.height=size+'px';ballX=cx+scrollX;ballY=cy+scrollY;ballVX=0;ballVY=90;obstacles=textRects();checkpoints=checkpointRects();floorY=document.documentElement.scrollHeight-8;stuckAnchorY=ballY;stuckSince=performance.now();lastInteractionAt=performance.now();hideIdleGuide();document.body.classList.add('game-running');ball.classList.add('on');ball.classList.remove('dropped','size-pop');ball.style.opacity='1';ball.style.transform=`translate3d(${cx-ballR}px,${cy-ballR}px,0)`;hint.textContent='←→ 横になぞる ｜ ↓ ページは自分でスクロール';hint.classList.add('show');setTimeout(()=>hint.classList.remove('show'),3000);void ball.offsetWidth;requestAnimationFrame(()=>ball.classList.add('dropped'));last=performance.now();setTimeout(()=>requestAnimationFrame(frame),160)}
function frame(now){if(!gameRunning)return;const dt=Math.min(.025,Math.max(.006,(now-last)/1000));last=now;if(steer){const dx=steerX-ballX;ballVX+=Math.max(-1100,Math.min(1100,dx*8.5))*dt}else if(key)ballVX+=key*1000*dt;else ballVX*=Math.pow(.988,dt*60);ballVY+=900*dt;ballVX*=Math.pow(.98,dt*60);ballVY=Math.min(ballVY,680);ballX+=ballVX*dt;ballY+=ballVY*dt;const left=ballR+5,right=Math.min(document.documentElement.clientWidth,720)-ballR-5;if(ballX<left){ballX=left;ballVX=Math.max(85,Math.abs(ballVX)*.48)}else if(ballX>right){ballX=right;ballVX=-Math.max(85,Math.abs(ballVX)*.48)}checkCheckpoints();for(const o of obstacles){if(o.b<ballY-ballR-70||o.t>ballY+ballR+70)continue;collide(o)}const gateY=scrollY+innerHeight*.74;if(ballY+ballR>gateY&&gateY<floorY-70){ballY=gateY-ballR;if(ballVY>0)ballVY=0}const g=goalHit();if(g){finish(g.gx,g.gy);return}if(ballY+ballR>=floorY){ballY=floorY-ballR;ballVY=-Math.max(460,Math.abs(ballVY)*.72);ballVX*=.9}ball.style.transform=`translate3d(${ballX-scrollX-ballR}px,${ballY-scrollY-ballR}px,0)`;updateIdleGuide(now);requestAnimationFrame(frame)}
