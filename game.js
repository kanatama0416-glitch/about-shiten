...
/* Sensor bridge used by tilt-fix.js. This is deliberately direct: sensor data
   changes the exact `tilt` value consumed by the physics loop. */
window.addEventListener('shiten-steer',e=>{
  const v=Number(e.detail);
  if(Number.isFinite(v))tilt=Math.max(-1,Math.min(1,v));
});
/* Desktop / sensorless fallback */
window.addEventListener('keydown',e=>{
  if(!gameRunning)return;
  if(e.key==='ArrowLeft')tilt=-1;
  if(e.key==='ArrowRight')tilt=1;
});
window.addEventListener('keyup',e=>{
  if(e.key==='ArrowLeft'||e.key==='ArrowRight')tilt=0;
});
window.addEventListener('pointermove',e=>{
  if(!gameRunning||e.pointerType==='touch')return;
  tilt=Math.max(-1,Math.min(1,(e.clientX-innerWidth/2)/(innerWidth*.35)));
},{passive:true});
...
function gameFrame(now){
  ...
  ballVX+=tilt*1450*dt;ballVY+=1080*dt;ballVX*=Math.pow(.982,dt*60);ballVY=Math.min(ballVY,820);ballX+=ballVX*dt;ballY+=ballVY*dt;
...
function finishGame(gx,gy){
  gameRunning=false;physicsStarted=false;tilt=0;const sx=gx-scrollX-ballR,sy=gy-scrollY-ballR;ball.style.transition='transform .34s cubic-bezier(.2,.9,.3,1),opacity .25s ease .2s';ball.style.transform=`translate3d(${sx}px,${sy}px,0)`;
