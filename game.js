...
body.game-running{overscroll-behavior:none;touch-action:none}body.game-running a{pointer-events:none}body.game-running .hero-pupil{opacity:0}body.game-running #goalPupil{opacity:0}
...
window.addEventListener('pointerdown',e=>{if(!gameRunning)return;steer=true;steerX=e.clientX+scrollX;if(e.cancelable)e.preventDefault()},{passive:false});
window.addEventListener('pointermove',e=>{if(!gameRunning||!steer)return;steerX=e.clientX+scrollX;if(e.cancelable)e.preventDefault()},{passive:false});
window.addEventListener('pointerup',e=>{if(!gameRunning)return;steer=false;if(e.cancelable)e.preventDefault()},{passive:false});
window.addEventListener('pointercancel',()=>{steer=false},{passive:true});
